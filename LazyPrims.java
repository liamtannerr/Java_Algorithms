...
/********************************************************************************************************************
 * Compilation and Execution: java -cp /path/to/lib.jar LazyPrims.java filename.txt
 *  -this works via single source-file mode: https://docs.oracle.com/en/java/javase/20/docs/specs/man/java.html
 * 
    ...
    private void lazy_prim(EdgeWeightedGraph G, int v) {
        ...
        scan(G, v);
            ...
            while (!pq.isEmpty()) {                        
                ...
                int i = e.either();
                int w = e.other(i);        
                assert marked[i] || marked[w];
                if (marked[i] && marked[w]) continue;      
                mst.enqueue(e);                            
                weight += e.weight();
                if (!marked[i]) scan(G, i);               
                if (!marked[w]) scan(G, w);               
            }
            // Credit to https://algs4.cs.princeton.edu/code/
